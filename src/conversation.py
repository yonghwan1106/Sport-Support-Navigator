...
class ConversationManager:
   ...
   def __init__(self):
       """
       대화 관리자를 초기화합니다.
       주요 상태 변수들과 로깅, 템플릿, 키워드 사전을 설정합니다.
       """
       # 기본 상태 변수 초기화
   ...
   def _get_alternative_question(self) -> str:
    ...
    def get_conversation_summary(self) -> Dict:
       ...
       Returns:
           Dict: 대화 요약 정보를 담은 딕셔너리 (프로필, 상태, 대화 길이, 완료 여부 포함)
       """
       return {
    ...
    def validate_profile(self) -> List[str]:
       ...
       필수 필드(사업 단계, 분야, 필요 지원)의 존재 여부를 검사합니다.
    ...
    def handle_error(self, error_type: str) -> str:
       ...
       각 오류 유형에 따라 적절한 안내 메시지를 생성합니다.
       
       Args:
           error_type: 오류 유형 ('duplicate', 'invalid', 'missing', 'system' 중 하나)
